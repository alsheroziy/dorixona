...
@section('header', 'Kategoriyani tahrirlash')
            ...
            <div class="p-6">
                <form action="{{ route('admin.categories.update', $category) }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="status" id="status" required
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                <option value="active" {{ old('status', $category->status) == 'active' ? 'selected' : '' }}>Faol</option>
                                <option value="inactive" {{ old('status', $category->status) == 'inactive' ? 'selected' : '' }}>Nofaol</option>
                            </select>
                            @error('status')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="mt-6 flex justify-end space-x-3">
                        <a href="{{ route('admin.categories') }}" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-300">
                            Bekor qilish
                        </a>
