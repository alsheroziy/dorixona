...
@section('title', 'Sotuvni tahrirlash')

@section('header', 'Sotuvni tahrirlash')
                ...
                <form action="{{ route('admin.sales.update', $sale) }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="payment_type" id="payment_type" required
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                <option value="cash" {{ old('payment_type', $sale->payment_type) == 'cash' ? 'selected' : '' }}>Naqd pul</option>
                                <option value="card" {{ old('payment_type', $sale->payment_type) == 'card' ? 'selected' : '' }}>Plastik karta</option>
                                <option value="transfer" {{ old('payment_type', $sale->payment_type) == 'transfer' ? 'selected' : '' }}>O'tkazma</option>
                            </select>
                            @error('payment_type')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <select name="status" id="status" required
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                <option value="completed" {{ old('status', $sale->status) == 'completed' ? 'selected' : '' }}>Yakunlangan</option>
                                <option value="pending" {{ old('status', $sale->status) == 'pending' ? 'selected' : '' }}>Kutilmoqda</option>
                                <option value="cancelled" {{ old('status', $sale->status) == 'cancelled' ? 'selected' : '' }}>Bekor qilingan</option>
                            </select>
                            @error('status')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="mt-6">
                        ...
                        <textarea name="notes" id="notes" rows="3"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('notes', $sale->notes) }}</textarea>
                        @error('notes')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
